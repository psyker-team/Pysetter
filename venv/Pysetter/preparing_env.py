...
def prepare_environment():
    ...
    install_torch = os.environ.get('INSTALL_TORCH', "False")
    torch_index_url = os.environ.get('TORCH_INDEX_URL', "https://download.pytorch.org/whl/cu121")
    torch_version = os.environ.get('TORCH_VERSION', "2.1.2")
    torchvision_version = os.environ.get('TORCHVISION_VERSION', "0.16.2")
    torch_command = f"pip install torch=={torch_version} torchvision=={torchvision_version} --no-cache-dir --no-warn-script-location --index-url {torch_index_url}"
    requirements_file = os.environ.get('REQS_FILE', "requirements.txt")

    install_xformers = os.environ.get('INSTALL_XFORMERS', "False")
